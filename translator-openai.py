...
def translate_chunk(chunk: str, glossary_text: str = "") -> str:
    system_prompt = (
    ...
    system_prompt += "Return only the translated Markdown text, no explanation."
...
def translate_code_block_comments(block: str, glossary_text: str = "") -> str:
    """只翻译代码块中的注释（//、#、/* */ 等），代码与标识符保持不变。"""
    system_prompt = (
...
def translate_markdown(text: str, *, glossary_text: str = "", verbose: bool = True) -> str:
    ...
    for chunk in chunks:
        if is_translatable_chunk(chunk):
            ...
            translated = translate_chunk(chunk, glossary_text=glossary_text)
    ...
    for i, block in enumerate(code_blocks):
        ...
        code_blocks_with_translated_comments.append(
            translate_code_block_comments(block, glossary_text=glossary_text)
        )
        time.sleep(REQUEST_INTERVAL_SECONDS)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Translate Markdown files from English to Simplified Chinese.")
    parser.add_argument(
        "--folder",
        type=Path,
        required=True,
...
def main() -> None:
    ...
    if glossary_text:
        print(f"Loaded glossary from {glossary_path} ({len(glossary_text.splitlines())} terms).")
    else:
    ...
    for index, src_file in enumerate(md_files, start=1):
        ...
        try:
            ...
            translated_text = translate_markdown(
                original_text, glossary_text=glossary_text, verbose=True
            )
